...
def save_measure_sheet():
    ...
    for col in header:
        if col == parent_dict['geography_id']:
            ...
            # Note: This will NOT work if the gau_id column contains ids from outside the parent row's Geography,
            # which is now allowed in the model. The problem is that if we go outside a single Geography,
            # we are not guaranteed that the names of the gau_ids are unique; they are re-used across Geographies.
            # In order to make this editing paradigm work with gau_ids from any Geography, the names will have
            # to be made unique across the whole GeographiesData table.
            geographies_data = _lookup_for('GeographiesData', 'name', 'geography_id',
...
def delete_measure():
    ...
    if sht.range((wb.app.selection.row, MEASURE_ID_COL)).value == SENSITIVTY_LABEL:
        _msg("Delete failed: can only delete measures, not sensitivities.")
    else:
        table = sht.range((wb.app.selection.row, MEASURE_TABLE_COL)).value
        parent_id = int(sht.range((wb.app.selection.row, MEASURE_ID_COL)).value)
        parent_id_col = _get_id_col_of_parent(table)
